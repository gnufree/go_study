...
func CreateFavoriteDir(favoriteDir *common.FavoriteDir) (err error) {
	...
	if err != nil {
		logger.Error("select dir_name failed, favoriteDir:%#v, err:%v", favoriteDir, err)
		return
	}
	if dirCount > 0 {
		tx.Rollback()
	...
	if err != nil {
		logger.Error("insert favorite_dir failed, favoriteDir:%#v, err:%v", favoriteDir, err)
		tx.Rollback()
		return
	}
	err = tx.Commit()
...
func CreateFavorite(favorite *common.Favorite) (err error) {
	...
	if err != nil {
		logger.Error("select dir_name failed, favorite:%#v, err:%v", favorite, err)
		return
	}
	if favoriteCount > 0 {
		tx.Rollback()
...
func GetFavoriteDirList(userId int64) (favoriteDirList []*common.FavoriteDir, err error) {
	...
	if err != nil {
		logger.Error("select favorite_dir failed, err:%v", err)
		return
	}
	return
}
