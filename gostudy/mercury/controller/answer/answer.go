...
func AnswerListlHandle(c *gin.Context) {
	...
	logger.Debug("get answer list parameter success, qid:%v, offset:%v, limit:%v",
		questionId, offset, limit)

	answerIdList, err := db.GetAnswerIdList(questionId, offset, limit)
	if err != nil {
		logger.Error("db.GetAnswerIdList failed, question:%v, offset:%v, limit:%v, err:%v",
